...
import { Download, ImageIcon, Search, Sparkles, X } from "lucide-react";
...
const categoryGradient: Record<string, string> = {
  写真: "from-blue-50 via-slate-50 to-white",
  商品: "from-slate-50 via-blue-50 to-white",
  角色: "from-sky-50 via-cyan-50 to-white",
  界面: "from-white via-blue-50 to-slate-50",
  建筑: "from-slate-50 via-white to-blue-50",
  插画: "from-cyan-50 via-blue-50 to-white",
  国风: "from-sky-50 via-slate-50 to-white",
...
function workToItem(work: GalleryImageView): ShowcaseItem {
  return {
    ...
    tags: [work.category, work.ratio, work.provider],
    ratio: work.ratio,
    provider: work.provider,
    imageUrl: work.url,
    thumbnailUrl: work.thumbnailUrl || work.url,
    gradient: categoryGradient[work.category] || categoryGradient.其他,
...
export function HomeWorksShowcase({
  ...
  const visibleItems = usingFallback ? filterItems(fallbackItems, query, category) : realItems;

  useEffect(() => {
    if (usingFallback && !query.trim() && category === "全部") {
      return;
    }
  ...
  return (
    <>
      <section className="rounded-[26px] border border-slate-200 bg-white/90 p-4 shadow-card backdrop-blur md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm font-bold text-slate-500">
              {usingFallback ? "暂无公开作品，先展示精选样例。" : "来自真实生成任务的公开作品。"}
            </p>
          </div>
          <label className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 lg:max-w-md">
            ...
            />
            {loading ? <span className="h-2 w-2 rounded-full bg-ocean-400" /> : null}
          </label>
        </div>
      ...
      {visibleItems.length > 0 ? (
            ...
            <div key={item.id} className="break-inside-avoid">
              ...
              >
                ...
                <div className={`relative ${item.heightClass} bg-gradient-to-br ${item.gradient} p-4`}>
                  <div className="absolute right-4 top-4 z-10 rounded-full border border-white/80 bg-white/75 px-3 py-1 text-xs font-black text-slate-600 backdrop-blur">
                    {item.ratio}
                  </div>
                  {item.thumbnailUrl ? (
                    <img
                      ...
                      alt={item.title}
                      className="h-full w-full rounded-[20px] border border-white/80 object-cover shadow-inner"
                    />
                  ) : (
                ...
                <div className="relative space-y-2 p-4">
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">{item.category}</p>
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white/90 p-8 text-center shadow-card backdrop-blur">
      ...
      {selectedItem
        ? createPortal(
            <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/18 p-3 backdrop-blur-[2px]" onClick={() => setSelectedItem(null)}>
              <div
                className="relative grid max-h-[calc(100vh-24px)] w-[min(940px,calc(100vw-24px))] overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-app md:grid-cols-[1.25fr_.9fr]"
              ...
              >
                ...
                <div className="flex min-h-[420px] items-center justify-center bg-slate-50 p-0">
                  {selectedItem.imageUrl ? (
                    <img src={selectedItem.imageUrl} alt={selectedItem.title} className="h-full max-h-[calc(100vh-24px)] w-full object-contain bg-slate-50" />
                  ) : (
                    <div className={`h-full min-h-[420px] w-full bg-gradient-to-br ${selectedItem.gradient} p-6`}>
                      <div className="flex h-full min-h-[420px] items-end rounded-[24px] border border-white/80 bg-white/38 p-6 shadow-inner backdrop-blur-sm">
                        <div>
                          ...
                          <h3 className="mt-2 text-4xl font-black text-slate-950">{selectedItem.title}</h3>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <aside className="max-h-[calc(100vh-24px)] space-y-4 overflow-y-auto p-5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-blue-100 to-slate-50 text-sm font-black text-ocean-700">
                  ...
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-600"
                  >
